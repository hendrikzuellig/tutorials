...
#
# Further adapted by Stephen Ibanez to build a more customized topology for
# experimenting with CS344 routers
#
...
class SumeTopo(Topo):
    """ The mininet topology class for experimenting with SimpleSumeSwitch routers.
    """
    ctrl_port = 1
    nf_port_map = {"nf0":2, "nf1":3, "nf2":4, "nf3":5}
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        for sw in switches.keys():
            ...
            # add local controller link
            ctrl_host = 'c{}'.format(sw[1:])
            self.addHost(ctrl_host)
            self.addLink(ctrl_host, sw, port2=SumeTopo.ctrl_port)

        for link in host_links:
            host_name = link['node1']
            sw_name = link['node2'].split('-')[0]
            sw_nf = link['node2'].split('-')[1]
            sw_port = SumeTopo.nf_port_map[sw_nf]
        ...
        for link in switch_links:
            sw1_name = link['node1'].split('-')[0]
            sw1_nf = link['node1'].split('-')[1]
            sw1_port = SumeTopo.nf_port_map[sw1_nf]
            sw2_name = link['node2'].split('-')[0]
            sw2_nf = link['node2'].split('-')[1]
            sw2_port = SumeTopo.nf_port_map[sw2_nf]
...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
                       switch_json, bmv2_exe='simple_sume_switch', quiet=False):
...
#        self.program_hosts()
    ...
    def parse_links(self, unparsed_links):
        ...
        for link in unparsed_links:
            ...
            sw_fmat = r'^s\d+-nf[0123]$'
            if link_dict['node1'][0] == 's':
                assert re.search(sw_fmat, link_dict['node1']) is not None, 'Links connected to switches must be specified using the format: sX-nfY, not {}'.format(link_dict['node1']) 
            if link_dict['node2'][0] == 's':
                assert re.search(sw_fmat, link_dict['node2']) is not None, 'Links connected to switches must be specified using the format: sX-nfY, not {}'.format(link_dict['node2']) 
    ...
    def create_network(self):
        ...
        self.topo = SumeTopo(self.hosts, self.switches, self.links, self.log_dir)
